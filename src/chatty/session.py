...
class ChatbotSession:
  ...
  def __init__(
    ...
    default_prompt = (
      ...
      "CRITICAL: For performing search-and-replace edits (similar to 'sed'), you MUST use 'patch_file'. It takes a 'patch' string containing one or more Aider-style SEARCH/REPLACE blocks (format: <<<<<<< SEARCH\n[exact lines to replace]\n=======\n[new lines]\n>>>>>>> REPLACE). The SEARCH block must match a unique sequence of consecutive lines in the target file exactly (including comments, blank lines, and indentation). Chaining multiple SEARCH/REPLACE blocks sequentially in a single 'patch' call is supported for editing multiple locations in the same file. Do not include line numbers, file paths, or diff headers inside the blocks.\n"
